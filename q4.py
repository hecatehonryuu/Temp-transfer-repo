...
city_cuisine = text_file.map(lambda line: line.split(",")).map(lambda x: (x[2], x[3], 1)).groupBy(lambda x: (x[0], x[1]))

count = city_cuisine.reduceByKey(lambda x, y: x[2] + y[2]).map(lambda x: (x[0][0], x[0][1], x[1]))
